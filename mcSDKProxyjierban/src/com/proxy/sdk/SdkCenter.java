...
public class SdkCenter {
	...
	public void onCreate(final Activity activity){
		m_activity = activity;
		setSdkProxy( SdkChannel.getInstance() ); //初始化SdkProxy类
		if(Util.getSplash(activity).equals("1")){  //进行闪屏条件判断一系列逻辑
			LogUtil.e("闪屏=="+Util.getSplash(activity));
			if(Util.getChannle(activity).equals("37wan")||Util.getChannle(activity).equals("")){
				sdkProxy.onCreate(activity); //不需要闪屏
			}else{
				//静态闪屏，获取打包系统配置闪屏图片
				if(kndata.getGameInfo().getGameId().equals("guhuozainew")){ //此游戏已停止运营，废弃了
