...
CQGAME.getServerList = function() {
	...
	var url = "http://data.u7game.cn/api.php?m=servers&game=" + CQGAME.game + "&channel=" + CQGAME.channel + "&channelid=" + CQGAME.adChannel + "&openid=" + CQGAME.openid;
	Zepto.ajax({
		url : url,
		type : 'get',
		dataType : 'jsonp',
	...
	Zepto("#server_my").click(CQGAME.showMyServers);
	Zepto("#server_index").click(CQGAME.showServerList);
}

CQGAME.showServerList = function() {
	Zepto(".cate > b").removeClass("cur");
	Zepto("#server_index").addClass("cur");


	Zepto("#serverList").html("");
	for(var i in CQGAME.serverList) {
		// {"server_id":100,"port":8888,"name":"名字","id":1,"status":1,"ip":"xx.xx.xx.xx"}
		var server_info = CQGAME.serverList[i];
...
CQGAME.getNotice = function() {
	...
	var url = "http://data.u7game.cn/api.php";
	var params = "a=index&app_id=" + CQGAME.appid + "&channel=" + CQGAME.adChannel + "&game=" + CQGAME.game + "&m=notice&serverid=1";
...
CQGAME.loadVersion = function() {
	...
	var url = window.GAME_RES_PATH + "ver.js?r=" + Math.random();
	Zepto.getScript(url , function() {
		if(SCRIPT_VERSIONS['VER'] && SCRIPT_VERSIONS['TIME']) {
			Zepto("#version_div").text('VER: ' + SCRIPT_VERSIONS['VER'] + ' , ' + SCRIPT_VERSIONS['TIME']);
		}
