...
import com.proxy.task.CommonAsyncTaskdoHeartbeat;
...
public class HttpService {
	...
	public static void applyOrder( Activity activity ,KnPayInfo knPayInfo, BaseListener listener) {
		try {
			...
			LogUtil.e("=======uid"+uid+"::::server_id"+server_id+"------gameInfo"+gameInfo.toString()+"    "+knPayInfo.getExtraInfo());
	...
	//发送等级url
	public static void enterGame(Activity activity,String sceneId,BaseListener listener) {
		try {
	...
	public static void doHtmlUrl(final Activity activity , Map<String,Object> data ,  BaseListener listener){
		...
		try{
			...
			String is_first = (String) data.get("is_first");
	...
	//在线心跳包协议(30s)一次：
	public static void doHeartbeat(Activity activity, BaseListener listener){
		try{
			...
			new CommonAsyncTaskdoHeartbeat(null,Constants.URL.HEARTBRAT, listener).execute(new Map[] { params, null, null });
		}catch(Exception e){
			e.printStackTrace();
		}
	}
	...
	public static void doCancel(Activity activity, String act_type, BaseListener listener) {
		try {
			...
			String url= Constants.URL.CANCEL;
			params.put("imei",msi );
			...
			new CommonAsyncTask(null,url, listener).execute(new Map[] { params, null, null });
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public static void doPushData( final  Activity activity, Map<String,Object> data , BaseListener listener){
		...
		try{
			...
			String game_id = (String) data.get("game_id"); //游戏名称
			params.put("app_key",(String) data.get("app_key"));
